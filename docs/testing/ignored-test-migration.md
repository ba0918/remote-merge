...
| cli_diff::test_diff_directory | Child differences omitted for directory argument | Keep | Normal CLI diff, child paths and content (`tests/cli_diff_general.rs`) |
| cli_diff::test_diff_dot_path_resolves | `.` fails to expand to the comparison tree | Keep | Normal CLI diff, affected file (`tests/cli_diff_general.rs`) |
| cli_diff::test_diff_empty_file | Two zero-byte files reported different | Keep, boundary of equal | Normal CLI diff, exit and output (`tests/cli_diff_general.rs`) |
...
| cli_diff::test_diff_left_only_file | Left-only content omitted from diff | Keep | Normal CLI diff, file and deletion content (`tests/cli_diff_general.rs`) |
| cli_diff::test_diff_max_lines | Diff ignores output line limit | Keep | Normal CLI diff, bounded lines from real changes (`tests/cli_diff_general.rs`) |
| cli_diff::test_diff_multiple_files | Only first requested file gets a diff | Keep | Normal CLI diff, both contents (`tests/cli_diff_general.rs`) |
| cli_diff::test_diff_nonexistent_file | Missing path treated as a successful comparison | Keep | Normal CLI diff, nonzero status and path (`tests/cli_diff_general.rs`) |
...
| cli_diff::test_diff_right_only_file | Right-only content omitted from diff | Keep | Normal CLI diff, file and added content (`tests/cli_diff_general.rs`) |
...
| cli_diff::test_diff_symlink | A link change compares target bytes rather than link destination | Fold into `diff_shows_link_targets_and_resolved_file_contents` | Normal CLI diff, link destinations and resolved content |
| cli_diff::test_diff_text_shows_unified_diff | Modified lines are missing from unified diff | Keep | Normal CLI diff, old/new lines (`tests/cli_diff_general.rs`) |
...
| cli_diff::test_diff_with_ref | Reference comparison ignores the third content | Keep | Normal CLI three-way diff, reference result (`tests/cli_diff_general.rs`) |
...
| contract::ssh_sudo::privileged_merge_preserves_root_ownership_and_backs_up_the_old_contents | Privileged write loses owner/mode or old contents | Fold into `sudo::permitted_sudo_preserves_root_owner_mode_and_original_backup`; fixture cannot model OS owner | Container CLI, owner, mode, bytes, backup |
| e2e_testenv::agent_setting_selects_the_requested_remote_transport | Agent toggle silently selects wrong transport | Keep, distinguish installed-agent and direct SSH effects | Container CLI, resulting bytes and agent deployment (`tests/container-e2e/tests/compat.rs`) |
| e2e_testenv::remote_merge_keeps_backups_outside_the_target_root | Merge pollutes remote root with backup | Keep, overlaps aggregate-store test but checks actual sshd | Container CLI, remote listing and aggregate contents (`tests/container-e2e/tests/compat.rs`) |
| e2e_testenv::rollback_follows_an_unchanged_symlink_root | Stable root symlink prevents legitimate restoration | Keep | Container CLI, resolved target original bytes (`tests/container-e2e/tests/compat.rs`) |
| e2e_testenv::rollback_preserves_existing_remote_owner_and_permissions | Restore changes existing owner or mode | Keep, actual OS metadata needed | Container CLI, metadata and bytes (`tests/container-e2e/tests/compat.rs`) |
| e2e_testenv::rollback_restores_through_an_intermediate_symlink_outside_the_root | Restore misses a symlinked file outside root | Keep | Container CLI, actual resolved file bytes (`tests/container-e2e/tests/compat.rs`) |
| e2e_testenv::rollback_skips_a_repointed_symlink_root | Restore writes into a new symlink target | Keep | Container CLI, rejected status and untouched new target (`tests/container-e2e/tests/compat.rs`) |
...
| tui_3way::test_3way_swap_with_x | Swap leaves sides unchanged | Keep | Normal PTY, changed side labels; `app::ref_swap::swap_exchanges_caches` checks exchanged contents |
...
| tui_merge::test_file_merge_with_m_and_confirm | Confirmed merge does not update destination | Fold; `m` no longer starts a tree merge | `confirming_a_tree_merge_copies_the_loaded_source_bytes` checks `R`, prompt and destination bytes |
...
| tui_merge::test_merge_on_equal_file_ignored | Equal file creates a needless write | Keep | Normal PTY, no-differences dialog and unchanged file |
| tui_merge::test_merge_undo_multiple_times | Multiple undo operations leave one merge applied | Fold; `u` reverses staged hunk edits, not completed file merges | `app::undo` and `app::hunk_ops` unit tests verify multiple staged snapshots and restored cache bytes |
| tui_merge::test_merge_undo_with_u | Undo leaves a merge applied | Fold; old test pressed `u` in tree pane, where it is not bound | `app::hunk_ops::test_apply_undo_roundtrip_preserves_content` verifies the staged edit round trip |
| tui_merge::test_sensitive_file_merge_shows_warning | Sensitive write proceeds without confirmation | Fold into `test_sensitive_file_merge_requires_confirmation`; the single-file dialog does not display a separate sensitive warning | Normal PTY, confirmation and unchanged destination after cancellation; CLI sensitive tests cover the separate `--force` boundary |
