# remote-merge 旧仕様書（移行元）

> 過去の構想と当時の進捗が混在する資料。現行の検証済み仕様として扱わない。
...
**SSH exec** でツリー取得・diff取得・ファイル転送（`cat >` による書き込み）。Agent プロトコル有効時は Agent 経由でファイル転送を行う。
...
[ssh]
timeout_sec              = 300    # 接続タイムアウト（デフォルト: 300秒）
strict_host_key_checking = "ask"  # known_hosts 検証（"yes" / "no" / "ask"、デフォルト: "ask"）

[scan]
max_scan_entries     = 100000  # ツリー走査の最大エントリ数（デフォルト: 100,000）
badge_scan_max_files = 5000    # バッジ計算の最大ファイル数（デフォルト: 5,000）
...
| オプション | 説明 |
|-----------|------|
| `--config <PATH>` | 設定ファイルのパスを明示指定 |
| `-y`, `--yes` | 確認プロンプトを全てスキップ（LLMエージェント向け） |
...
remote-merge status --max-entries 50000        # スキャン最大エントリ数を指定

# 特定ファイル・ディレクトリの差分
remote-merge diff src/config.ts --left local --right develop
remote-merge diff src/config.ts --left local --right develop --ref staging  # 3way diff
...
remote-merge merge src/config.ts --left develop --right local --ref staging  # 3way マージ
...
remote-merge events --since 1h --tail 20
...
| リクエスト | レスポンス | 用途 |
|-----------|-----------|------|
| `ListTree { root, exclude, include, max_entries }` | `TreeChunk { nodes, is_last, total_scanned, truncated }` | ディレクトリツリー走査（ストリーミング） |
...
merge・sync・rollback・TUI の書き込み前に作るバックアップは、実行したマシンの
ローカル集約先で管理する。書き込み先の root_dir 配下にはバックアップを作らない。

| 項目 | 内容 |
|------|------|
| 保存先 | `$XDG_DATA_HOME/remote-merge/backups/`。`XDG_DATA_HOME` が未設定・空・相対パスなら `~/.local/share/remote-merge/backups/` |
| 区別 | リモートは host・port・root_dir、ローカルは root_dir の絶対パスで書き込み先を区別 |
| 権限 | ディレクトリは 0700、ファイルは 0600 |
| 自動削除 | TUI 起動時と、dry-run でない merge・sync の開始時に、現在の設定にある書き込み先の期限切れセッションを削除。rollback は削除しない |
...
バックアップが有効なとき、バックアップを保存できないファイルには書き込まない。
新規作成するファイルには元の内容がないためバックアップを作らない。集約先の中身を
手で削除しても本体の動作には影響せず、記録が欠けたセッションは一覧にも復元対象にも
含めない。書き込み先ごとの保存領域には、対象を識別できる host・port・root_dir
（ローカルでは root_dir の絶対パス）を人が読める形式で記録する。
...
サーバ間マージはマージ先サーバに SSH exec (`cat >`) または Agent プロトコル経由で直接書き込みを行う。
...
読み取り操作では接続エラー時に1回自動リトライする。書き込み操作（マージ等）では自動再接続は行わない。
...
バックアップは、実行したマシンのローカル集約先に、書き込み先と操作ごとの
セッションを区別して保存する。集約先は `$XDG_DATA_HOME/remote-merge/backups/`、
`XDG_DATA_HOME` が未設定・空・相対パスなら
`~/.local/share/remote-merge/backups/` になる。内部のディレクトリ構成と記録形式は
公開契約ではない。

1 回の書き込み操作につき、開始時にセッション ID を 1 回だけ決める。ID は UTC の
`YYYYMMDD-HHMMSS` に、同じ秒の ID が既にある場合だけ `-N`（N は 2 以上）を付ける。
例は `20240115-140000`、`20240115-140000-2`。ID は集約先全体で重複せず、sync は
全書き込み先で同じ ID を使う。何も記録しなかったセッションは残さない。
...
| 復元部分成功（一部スキップ・失敗） | 2 | symlink 保護などでスキップしたか、一部ファイルの復元に失敗 |
...
| 項目 | 内容 |
|------|------|
| 対象 | `--target` が現在指す書き込み先について、ローカル集約先にある完全なセッション |
| デフォルト | 直近の non-expired セッションを一括復元 |
| 確認 | 復元対象を一覧表示し、確認プロンプトを表示（`--force` で省略可） |
| 安全策 | 復元直前にパスを辿り直し、バックアップ時と実際の書き込み先が違う場合や symlink 自体の復元になる場合はスキップ。復元前の既存ファイルは新セッションへ退避 |
| sensitive | merge と同様に `is_sensitive()` でフィルタし、`--force` なしではスキップ |
| expired | セッション ID の UTC 日時から `retention_days` × 24 時間以上経過すると `--list` で `[expired]` 表示。復元はブロックされ、`--force` で許可 |

### 制約事項

- バックアップの保持期間（`retention_days`）以上経過したセッションは `--force` なしで復元不可
- `--force` でも symlink を壊す復元は行わない
- TUI の `u` キーは Diff View 内のハンクマージ undo（既存機能）であり、rollback とは別物
- セッション ID は `YYYYMMDD-HHMMSS` または `YYYYMMDD-HHMMSS-N`（N は 2 以上）を受け付ける
- rollback はバックアップを作成したマシンからだけ実行できる
...
│   │   ├── remote_io.rs     #   リモート I/O（SSH exec / Agent）
