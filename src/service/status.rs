...
///
/// コンテンツ比較は行わず、ツリー上の存在チェックのみ。
/// 両方に存在するファイルは `Modified`（コンテンツ未比較）として返す。
/// 正確な Modified/Equal 判定にはコンテンツ読み込みが必要。
pub fn compute_status_from_trees(
    ...
    for path in &all_paths {
        ...
        let status = match (left_presence, right_presence) {
            (NodePresence::Found, NodePresence::Found) => FileStatusKind::Modified,
