...
use crate::app::search::{
    find_matches, format_search_status, format_search_status_with_pos, nearest_match_from,
...
pub fn handle_search_key(state: &mut AppState, code: KeyCode) {
    match code {
        KeyCode::Char(c) => {
            state.search_state.query.push(c);
            state.rebuild_flat_nodes();
            jump_to_first_match(state);
        }
        KeyCode::Backspace => {
...
pub fn jump_next(state: &mut AppState) {
    ...
    jump_to_match(state, &matches);
}

/// 検索結果の前の一致にジャンプする（検索モード外でも動作）。
pub fn jump_prev(state: &mut AppState) {
    if !state.search_state.has_query() {
        return;
    }
    let matches = find_matches(&state.flat_nodes, &state.search_state.query);
...
fn jump_to_first_match(state: &mut AppState) {
    let matches = find_matches(&state.flat_nodes, &state.search_state.query);
    if matches.is_empty() {
        update_search_status(state, 0);
        return;
...
fn jump_to_match(state: &mut AppState, matches: &[usize]) {
...
fn update_search_status(state: &mut AppState, total: usize) {
    state.status_message = format_search_status(&state.search_state.query, total);
}

/// 検索ステータスメッセージを更新する（位置付き）。
fn update_search_status_with_pos(state: &mut AppState, pos: usize, total: usize) {
    state.status_message = format_search_status_with_pos(&state.search_state.query, pos, total);
}
...
mod tests {
    ...
    fn test_char_input_updates_query() {
        ...
        handle_search_key(&mut state, KeyCode::Char('m'));
    ...
    fn test_enter_deactivates_keeps_query() {
        ...
        handle_search_key(&mut state, KeyCode::Enter);
    ...
    fn test_esc_clears_search() {
        ...
        handle_search_key(&mut state, KeyCode::Esc);
        assert!(!state.search_state.active);
